...
public class PushNotificationOutboxPoller extends BaseOutboxPoller {
    ...
    public void poll() {
        ...
        List<TicketOutbox> entities = ticketOutboxRepository.findAllMessagesNotSent(Channel.PUSH_NOTIFICATION, chunkSize);
        if (entities.isEmpty()) {
            return;
        }
        ...
        sqsTemplate.sendManyAsync(sqsQueueName, messageEntries)
                .whenCompleteAsync(getOutboxSendResultHandler(Channel.PUSH_NOTIFICATION));
    }
}